...
async def update_titles():
    ...
    try:
        ...
        df_base = pd.read_csv(StringIO(res_base.text), encoding="utf-8")
        for _, row in df_base.iterrows():
            tg_id = extract_id(row.iloc[0])
            if tg_id:
                valid_chat_members.add(str(tg_id))
    except Exception as e:
    ...
    try:
        ...
        for _, row in df_form.iterrows():
            ...
            if f_tg_id and f_v5_id and f_tg_id in valid_chat_members:
                if str(TARGET_CHAT_ID) not in cache:
                    cache[str(TARGET_CHAT_ID)] = {}
    ...
    async with bot:
        for tg_id, v5_id in cache.get(str(TARGET_CHAT_ID), {}).items():
            try:
                ...
                run = {"run500": "500", "run250": "250", "run100": "100",
                       "run50": "50", "run25": "25", "run10": "10"}.get(m.get("run"), "")
                vol = {"vol500": "500", "vol250": "250", "vol100": "100",
                       "vol50": "50", "vol25": "25", "vol10": "10"}.get(m.get("volunteer"), "")
...
def get_vk_photo(display_date, run_num):
    ...
    try:
        ...
        if target:
            ...
            p_img = {"owner_id": -VK_GROUP_ID, "album_id": target['id'],
                     "access_token": VK_TOKEN, "v": "5.131", "count": 1}
            photos = requests.get("https://api.vk.com/method/photos.get", params=p_img).json()\
...
async def check_new_vk_members():
    ...
    try:
        ...
        new_names = [
            f"<a href='https://vk.com/id{m['id']}'>{m['first_name']} {m['last_name']}</a>"
        ...
        with open(VK_MEMBERS_FILE, "w", encoding="utf-8") as f:
            json.dump([m['id'] for m in current_members], f)

        if new_names or not old_ids:
            git_push()

    except Exception as e:
        logger.error(f"Ошибка проверки новых участников ВК: {e}")


# ========================= ЕЖЕНЕДЕЛЬНЫЕ ИТОГИ =========================
async def send_weekly_stats():
    headers = login_5verst()
    if not headers or not VK_TOKEN:
        return
    ...
    async with bot:
        ...
        vk_r = requests.get(
            "https://api.vk.com/method/groups.getMembers",
            params={"group_id": VK_GROUP_ID, "access_token": VK_TOKEN, "v": "5.131", "count": 0}
        ).json()
